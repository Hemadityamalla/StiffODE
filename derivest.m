function [der,errest,finaldelta] = derivest(fun,x0,depvar,varargin)
...
for i = 1:n
  ...
  if par.Style(1) == 'c'
    ...
    if strcmpi(par.Vectorized,'yes')
      f_plusdel = fun(depvar,x0i+h*delta);
      f_minusdel = fun(depvar,x0i-h*delta);
    else
      % not vectorized, so loop
      f_minusdel = zeros(size(delta));
      f_plusdel = zeros(size(delta));
      for j = 1:numel(delta)
        f_plusdel(j) = fun(depvar,x0i+h*delta(j));
        f_minusdel(j) = fun(depvar,x0i-h*delta(j));
      end
    end
  ...
  elseif par.Style(1) == 'f'
    ...
    if strcmpi(par.Vectorized,'yes')
      f_del = fun(depvar,x0i+h*delta) - f_x0(i);
    else
      % not vectorized, so loop
      f_del = zeros(size(delta));
      for j = 1:numel(delta)
        f_del(j) = fun(depvar,x0i+h*delta(j)) - f_x0(i);
      end
    end
  else
    % backward rule
